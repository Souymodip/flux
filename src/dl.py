...
class DS(Dataset):
    def __init__(self, folder_path, shuffle, size):
...
def get_train_loader(folder_path, size, batch_size, num_workers):
    assert os.path.exists(folder_path), f"Folder path {folder_path} does not exist"
    ds = DS(folder_path, shuffle=True, size=size)
    return DataLoader(ds, batch_size=batch_size, shuffle=True, num_workers=num_workers, persistent_workers=True)


def get_val_loader(folder_path, size, batch_size, num_workers):
    assert os.path.exists(folder_path), f"Folder path {folder_path} does not exist"
    ds = DS(folder_path, shuffle=False, size=size)
    return DataLoader(ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, persistent_workers=True)


def test():
    import matplotlib.pyplot as plt
    folder_path = '/Users/souymodip/GIT/pythonProject/data'
    train_loader = get_train_loader(folder_path=folder_path, size=32, batch_size=256, num_workers=4)
